...
apt-get -y install php7.4-fpm > /dev/null 2>&1
...
apt-get -y install --yes --allow-unauthenticated vim nano net-tools \
	...
	php-mysqli php-mysql php7.4-mysql \
	php-xml php7.4-xml php-soap php7.4-soap php-zip php7.4-zip > /dev/null 2>&1
...
TS=$(date +'%T')
SSH_COPIED_PATH="${PROJECT_ROOT}/storage/container_id_rsa_7.4_${DATE}_${TS}"
